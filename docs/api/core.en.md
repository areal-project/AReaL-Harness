...
With `max_completion_retries`, classified transport, rate-limit, unavailable, truncated-stream or empty-terminal responses can recover within a bounded budget (reasoning alone is not a final answer). Core audits and removes the current incomplete response from model input, emits `areal/model/completionDiscarded`, and retains previously executed tools, steer input and usage. Tools execute only after a complete successful stream. UNKNOWN, unclassified errors, cancellation and timeouts are not replayed. The default zero budget disables recovery.

Compaction retains the original goal and recent content without splitting completion/tool-result or opaque-reasoning boundaries. Summaries are at most 16 KiB with throughItemId and a persisted checkpoint. Empty or pseudo-tool summaries get one retry; subsequent failure permits explicitly marked DEGRADED CONTEXT only if it reduces input, otherwise the Turn fails. Cancellation preserves the old checkpoint. Compaction never deletes history, journals or Turn tool state.
